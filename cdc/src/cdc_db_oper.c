...
static inline void get_voss_tablename(char *ip, char *stime, char *fname, char *tablename, int len, int role)
{
	if (role != ROLE_HOT_CS)
	{
		...
		if(!strncmp(curtime, stime, 8))
		{
			...
			snprintf(tablename, len, "t_ip_task_info_%.8s", curtime);
	}
	else
		snprintf(tablename, len, "t_hotip_task_info");
}
...
static int get_last_status(t_voss_key *k, t_voss_val *v, char *table)
{
	char sql[1024] = {0x0};

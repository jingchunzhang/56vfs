...
static uint32_t get_a_good_ip(t_cs_dir_info * csinfos, int isp, t_vfs_taskinfo *task)
{
	if (csinfos->index == 0)
	{
		LOG(vfs_sig_log, LOG_ERROR, "cs_dir index %d\n", csinfos->index);
		return 0;
	}
	uint32_t ip = 0;
	int i= 0;
	...
	for (i = 0; i < csinfos->index; i++)
	{
		...
		{
			LOG(vfs_sig_log, LOG_ERROR, "dispatch get_ip_info %u err %m\n", csinfos->ip[i]);
			continue;
		}
		if (ipinfo->offline)
		{
			LOG(vfs_sig_log, LOG_ERROR, "dispatch %u offline\n", csinfos->ip[i]);
			continue;
		}
		if (ip == 0)
		{
...
static void do_dispatch_to_cs_isp(int isp, t_task_base *base)
{
	t_vfs_tasklist *task = NULL;
...
static void do_dispatch_to_cs(t_vfs_tasklist *tasklist)
{
	t_task_base * base = (t_task_base *)(&(tasklist->task.base));
	base->starttime = time(NULL);
	do_dispatch_to_cs_isp(TEL, base);
	do_dispatch_to_cs_isp(CNC, base);
}
...
static void check_task()
{
	...
	while (1)
	{
		...
		if (g_config.del_tracker)
		{
			do_dispatch_to_cs(tasklist);
			continue;
		}

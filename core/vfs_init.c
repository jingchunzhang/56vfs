...
static t_cs_dir_info  csinfo[30][30];
...
void reload_config()
{
	...
	g_config.del_tracker = myconfig_get_intval("del_tracker", 0);
	LOG(glogfd, LOG_NORMAL, "del_tracker = %u\n", g_config.del_tracker);
}
...
static void pushcs(t_cs_dir_info *cs, char *sip, uint8_t isp, int d1, int d2)
{
	sip = strtrim(sip);
	if (sip == NULL)
	{
...
static void pushinfo(t_cs_dir_info *cs, char *ips, uint8_t isp, int d1, int d2)
{
	char *s = ips;
	char *e = NULL;
	while (1)
	{
		...
		*e = 0x0;
		e++;
		pushcs(cs, s, isp, d1, d2);
		s = e;
	}
	pushcs(cs, s, isp, d1, d2);
}

static int process_csfile(uint8_t isp, char *csfile)
{
	...
	while (fgets(buf, sizeof(buf), fp))
	{
		...
		pushinfo(cs, t1+1, isp, dir1, dir2);
	}
	fclose(fp);
	return 0;
}
...
static int init_fcs()
{
	char *fcsfile = myconfig_get_value("iplist_fcsfile");
	if (fcsfile == NULL)
	{
		LOG(glogfd, LOG_ERROR, "no iplist_fcsfile!\n");
		return -1;
	}
	...
	while (fgets(buf, sizeof(buf), fp))
	{
		...
		fcslist[atoi(ipinfo.sip+3)%MAXFCS] = 1;
		ipinfo.ip = ip;
		add_ip_info(&ipinfo);
	}
	fclose(fp);
...
static int init_csinfo()
{
	...
	while((dirp = readdir(dp)) != NULL) {
		...
		char *t = strstr(dirp->d_name, ".txt");
		if (!t)
			continue;
		if (strcmp(dirp->d_name + strlen(dirp->d_name) -4, ".txt"))
			continue;
		t = strstr(dirp->d_name, "new_cs_");
		if (!t)
			continue;

		isp = UNKNOW_ISP;
		char *t1 = strstr(dirp->d_name, "_mp4");
		if (t1)
			isp = MP4;
		else
		{
			t += 7;
			if (strncmp(t, "cnc.", 4) == 0)
				isp = CNC;
			else if (strncmp(t, "edu.", 4) == 0)
				isp = EDU;
			else if (strncmp(t, "yd.", 3) == 0)
				isp = YD;
			else if (strncmp(t, "hs.", 3) == 0)
				isp = HS;
			else if (strncmp(t, "tt.", 3) == 0)
				isp = TT;
			else if (strncmp(t, "tel.", 4) == 0)
				isp = TEL;
		}
		if (isp == UNKNOW_ISP)
			isp = get_isp(t);
		...
		snprintf(fullfile, sizeof(fullfile), "%s/%s", csdir, dirp->d_name);
		LOG(glogfd, LOG_NORMAL, "process cs %s\n", fullfile);
		if (process_csfile(isp, fullfile))
		{
			ret = -1;
			break;
		}
...
int get_isp(char *t)
{
	...
	for (; i < 256; i++)
	{
		int l = strlen(add_cs_isps[i]);
		if (l == 0)
			continue;
		if (strncmp(t, add_cs_isps[i], l) == 0)
			return i;
	}
	return UNKNOW_ISP;
}
...
int get_cs_info(int dir1, int dir2, t_cs_dir_info *cs)
{
	...
	memcpy(cs, &(csinfo[dir1%30][dir2%30]), sizeof(t_cs_dir_info));
	if (pthread_rwlock_unlock(&init_rwmutex))
		LOG(glogfd, LOG_ERROR, "ERR %s:%d pthread_rwlock_unlock error %m\n", FUNC, LN);
	return 0;
}

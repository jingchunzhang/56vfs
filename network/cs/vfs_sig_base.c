...
/* 同步逻辑如下：获取同组CS服务器状态 有正常服务的，向其发起同步请求
 * 没有同步的，判断自己是否是本组同运营商第一个机器，是则通过Tracker发起同同步请求，不是第一个IP的，等待
 *如果有本组机器状态为未知的，在下个周期进来重做上述逻辑
 */
static void do_active_sync()
{
	int run_sync_dir_count = get_task_count(TASK_Q_SYNC_DIR_TMP) + get_task_count(TASK_Q_SYNC_DIR_TMP);

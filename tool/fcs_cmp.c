...
static int fcs_cmp()
{
	...
	char sql[1024] = {0x0};
	snprintf(sql, sizeof(sql), "select domainname, first_ip from t_server_base_info where domainname like '%s' and groupid = 1 and status = 1 group by domainname order by  domainname;", like);
    if (mysql_query(mysql, sql))
	{
